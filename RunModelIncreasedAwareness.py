...
csv_rows = \
    [['Increase in Awareness (pp)', 'Suicide Rate', 'Suicide Rate 95% CI', 'Rate Reduction vs. Baseline',
...
csv_rows.append([increase_awareness, suicide_rate, suicide_rate_CI, reduction_from_baseline, reduction_from_baseline_CI,
...
for i in range(1,4):
    ...
    # simulate a cohort using the matrix (set IDs to i+3 so they don't match IDs from increased access exercise)
    cohort = Cohort(id=i+3, pop_size=D.POP_SIZE, transition_prob_matrix=matrix_support.transProbMatrix)
    cohort.simulate()

    # record cohort's increase in awareness, suicide rate, and suicide rate CI
    increase_awareness = i*4
    ...
    # add results to CSV
    csv_rows.append(
        [increase_awareness, suicide_rate, suicide_rate_CI, reduction_from_baseline, reduction_from_baseline_CI,
         reduction_from_previous, reduction_from_previous_CI]
        )
...
IO.write_csv(file_name='IncreasedAwareness.csv', rows=csv_rows)